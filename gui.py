...
def button():
    ...
    text.delete("1.0" , "end")
    midi_datas =[s for s in glob("midi/*")]


    for i in range(len(midi_datas)):
        file_list = []
        path = os.getcwd()
        midi_path = os.path.join(path, midi_datas[i])
        file_list.append(midi_path)
        ...
        def melody_form(midi_path):
            ...
            for track in midi_obj.instruments:
                    ...
                    if "melody" in track.name :
                        for note in track.notes:
                            if note.velocity !=1 :
                                result.append([note.pitch , note.start])
                    else:
                        return "-------------------"
            ...
            for _ in range(len(result)):
                if 0 < len(result):
                    index_1 = 0
                    index_2 = 1
                    if result[index_1][index_2] == result [index_1+1][index_2] and result[index_1][index_1]+12 == result[index_1+1][index_1] or result[index_1][index_1] == result[index_1+1][index_1]+12 :

                        new_list.appendleft(result[index_1][index_2])
                        del result[index_1]
                        del result[index_2-1]
                    else:
                        index_1 += 1
                        index_2 += 1
        ...
        for f in file_list:
            for file_name in os.listdir(f) :
                if file_name.endswith(".mid"):
                    ...
                    melody_form = melody_form(tmp_path) , file_name
                    ...
                    melody_form_result = str(melody_form)
                    ...
                    text.insert(tkinter.INSERT , "melody : " + melody_form_result + "\n")
